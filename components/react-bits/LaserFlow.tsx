...
export const LaserFlow: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    const setSizeNow = () => {
      ...
      // Reduce vertical sizing on mobile so the beam doesn't fly off-screen too far
      const responsiveVSize = isMobile
        ? Math.min(verticalSizing, 1.2)
        : verticalSizing;
      const responsiveHSize = isMobile
        ? Math.min(horizontalSizing, 0.4)
        : horizontalSizing;
      ...
      if (Math.abs(w - last.width) > 0.5 || Math.abs(h - last.height) > 0.5) {
        ...
        // Update these values dynamically on resize
        uniforms.uVLenFactor.value = responsiveVSize;
        uniforms.uHLenFactor.value = responsiveHSize;
